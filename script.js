...
if (value >= 1e6) {
  return (value / 1e6).toFixed(1) + " million"; // Formate en millions
  } else if (value >= 1e3) {
...
tooltip.style("opacity", 1)
  .html(`<strong>${d.year}</strong>: ${formatNumber(d.travellers)} of tourists`); // Contenu du tooltip
   d3.select(this).style("fill", "#FEF6B5");
})
  ...
  d3.select(this).style("fill", "#C8E6C9");
});
...
// Ajouter les labels des axes x et y
 svg.append("text")
  ...
  .text("Number of tourists (millions)")
  .style("font-family", "Montserrat Alternates, sans-serif")
  .style("font-size", "1rem")
  .style("font-weight", "bold")
  .style("fill", "#FFFFFF");

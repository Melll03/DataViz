...
d3.json('Datavisitors.json').then(data => {
  ...
  chart.selectAll(".bar")
      ...
      .style("fill", "#FFBC40") // Couleur initiale
      .style("transform-origin", d => `${x(d.year) + x.bandwidth() / 2}px ${height}px`)
      .on("mouseover", function(event, d) {
          tooltip.style("opacity", 1)
              .html(`<strong>${d.year}</strong>: ${formatNumber(d.travellers)} de touristes`);

          // Agrandir la barre survolée
          d3.select(this)
              .style("transform", "scale(1.3)")
              .style("fill", "#3C6E70") // Couleur plus foncée pour la barre survolée
              .style("z-index", 10); 
